...
for CELL_SIZE in [8,16]:
    for BATCH_SIZE in [32]:
        ...
        def create_model(x_train, y_train, x_test, y_test, file, params=None):
            ...
            # Conv Layer 1
            #cnn
            model.add(Conv1D(CELL_SIZE, kernel_size=2, activation='relu', padding='same', input_shape=input_shape))
            #dnn
            #model.add(Dense(CELL_SIZE, activation='relu', input_shape=input_shape))
        ...
        def read_data(n_fold, file='run.txt', type='cnn'):
            ...
            smo = SMOTE(random_state=42, sampling_strategy=1)
            ...
            if type == 'rnn':
                ...
                if 'run' in file:
                    ...
                    feature_set_SMOTED = feature_set_SMOTED.reshape((feature_set_SMOTED.shape[0], 279, 1))
                    test_size = 0.5
                    random_state = 42
                if '锰' in file:
        ...
        for i in range(n_fold):
            ...
            result.loc[:, "%d_fold" % (i+1)] = score + [con_mat[0, 0] / np.sum(con_mat, axis=1)[0],
